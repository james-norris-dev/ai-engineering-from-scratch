...
| 2026-08-10 | Phase 1, Lesson 02 (Vectors, Matrices & Operations) | 3/3 | Mixed up element-wise multiply and matrix multiply once (tried to sum everything into one number for both), corrected cleanly with side-by-side worked examples. Hit two more real bugs building the neural-network-layer demo: Matrix class was missing `__add__` (never added it this session, so `weights.matmul(inputs) + bias` threw TypeError), plus a `[0, 2]` vs `[0.2]` typo that would've broken the (3,1) input shape — both self-diagnosed from the traceback and fixed. Nailed identity-matrix and determinant/inverse predictions immediately.
